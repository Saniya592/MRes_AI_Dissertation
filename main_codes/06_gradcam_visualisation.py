...
"""
06_gradcam_visualisation.py

Stage 6:
Generate Grad-CAM visualisations.

This updated script focuses on misclassified cases.

It creates Grad-CAM images for:
- cases where baseline is wrong but transfer is right
- cases where transfer is wrong but baseline is right
- cases where both models are wrong
- high-confidence misclassified cases

It also creates a CSV list for appendix use.
"""
...
# Generate guidance for results in dissertation, it helps in selecting representative Grad-CAM cases.

    text = f"""
Grad-CAM appendix guidance

Total selected cases: {len(selected_cases)}
Total saved appendix rows: {len(appendix_df)}

Recommended use in report:
1. Use one baseline-wrong-transfer-right example to show where transfer improved.
2. Use one both-models-wrong example to show why five-class grading remains difficult.
3. Use one transfer-wrong-high-confidence example to discuss risk, uncertainty, borderline cases, noisy labels, or visually similar classes.

Main interpretation:
Grad-CAM helps show where the model focused during correct and wrong predictions. It supports error inspection, but it does not prove clinical reasoning by itself.
""".strip()

    (REPORTS_DIR / "gradcam_appendix_guidance.txt").write_text(text, encoding="utf-8")
